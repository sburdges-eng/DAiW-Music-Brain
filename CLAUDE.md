...
- The CLI commands are **implemented** with full subcommand support.
- The Intent Schema system exists alongside the simpler `TherapySession` approach.

If something in this document doesn't match the code, **trust the code**.
...
- `HarmonyPlan` - Blueprint for generation (mode, tempo, chords, structure_type, vulnerability)
...
session.set_scales(motivation: int, chaos_tolerance: float) -> None  # Note: chaos_tolerance
...
class HarmonyPlan:
    ...
    vulnerability: float = 0.5  # 0.0 - 1.0
...
session.set_scales(motivation=8, chaos_tolerance=0.6)  # Note: chaos_tolerance
