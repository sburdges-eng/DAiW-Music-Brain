...
"""
Tension Curve Generator - Bar-wise tension multipliers for emotional contour.

Generates 1D arrays of tension multipliers (~0.5 to ~1.5) for each bar,
acting as automation lanes for velocity and complexity modulation.

Philosophy: Music should have emotional shape, not just static intensity.
The difference between a loop pack and an actual writing partner is
emotional contours, not just emotional states.
"""
...
def generate_tension_curve(total_bars: int, structure_type: str = "standard") -> np.ndarray:
    ...
    Args:
        total_bars: Number of bars in the song
        structure_type: The narrative arc type
            - "climb"    : slow linear build (post-rock, endless build)
            - "standard" : verse/chorus/bridge shape
            - "constant" : flat line, loop/techno

    Returns:
        numpy array of tension multipliers, one per bar
    """
    if total_bars <= 0:
    ...
    if structure_type == "climb":
        ...
        # Unknown structure type → flat line
        curve[:] = 1.0
